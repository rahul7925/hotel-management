...
const createHotel = async (req, res) => {
    try {
        ...
        const result = await pool.query(
            "INSERT INTO hotels (title, description, latitude, longitude, price) VALUES ($1, $2, $3, $4, $5) RETURNING *",
...
const getAllHotels = async (req, res) => {
    try {
        ...
        hotelValues.push(offset);
        const offsetPosition = hotelValues.length;

        const result = await pool.query(
            'SELECT * FROM hotels ' + whereClause + ' ORDER BY id DESC LIMIT $' + limitPosition + ' OFFSET $' + offsetPosition,
            hotelValues
        );
